...
var jqCronDefaultSettings = {
	texts: {},
	monthdays:  Array.from({ length: 31 }, (v, i) => i+1),
...
(function($){
	...
	function jqCron(settings) {
		...
		var _self         = this;
		var _$elt         = this;
		var _$obj         = $('<span class="input-group input-group-sm"></span>');
		var _$blocks      = $('<span class="jqCron-blocks d-inline-flex align-items-center"></span>');
		var _$blockPERIOD = $('<span class="jqCron-period d-inline-flex align-items-center"></span>');
		...
		var _$cross       = $(`<span class="ms-2 jqCron-text"><svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" class="bi bi-arrow-clockwise ms-1" viewBox="0 0 16 16"><path fill-rule="evenodd" d="M8 3a5 5 0 1 0 4.546 2.914.5.5 0 0 1 .908-.417A6 6 0 1 1 8 2v1z"/><path d="M8 4.466V.534a.25.25 0 0 1 .41-.192l2.36 1.966c.12.1.12.284 0 .384L8.41 4.658A.25.25 0 0 1 8 4.466z"/></svg></span>`);
		var _selectors    = [];
		var _periods      = ['hour', 'day', 'week', 'month', 'year'];
		...
		this.isDisabled = () => settings.disabled == true;
		...
		this.getPeriod = ()=> _selectorPeriod.$;
		this.periodValue = () => _selectorPeriod.getValue();
		this.setPeriod = function(value) {
			if (_periods.includes(value)) {
				_selectorPeriod.setValue(value);
			}
		};
		this.syncPeriod = (other) => {
			_selectorPeriod.$.bind('selector:change', function(e, v){
				$(other).jqCronGetInstance().setPeriod(v)
			});
		}
	...
	function jqCronSelector(_cron, _$block, _multiple, _type){
		var _self      = this;
		var _$list     = $('<ul class="jqCron-selector-list dropdown-menu"></ul>');
		...
		this.getValue = () => _multiple ? _value : _value[0];
		...
		this.isOpened = () => _$list.is(':visible');
		...
		this.add = function(key, value) {
			...
			var $item = $('<li class="dropdown-item">' + value + '</li>');
			_$list.append($item);
			_values[key] = $item;
